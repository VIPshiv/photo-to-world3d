...
export default function StitchPage() {
  ...
  const isGuidedValid =
    Object.values(guidedImages).filter((img) => img !== null).length >= 2;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "cubemap") {
      ...
      if (guidedMode === "manual") {
        Object.values(guidedImages).forEach(
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-black font-sans p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-10">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Stitch multiple photos into a seamless 3D world.
            </p>
          </div>
          <div className="flex flex-col items-end gap-2">
            <Link
              href="/"
              className="px-4 py-2 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-100 hover:text-black transition-colors"
            >
              &larr; Back to Home
            </Link>
            <Link
              href="/upload"
              className="px-4 py-2 rounded-xl text-sm font-medium bg-gray-900 text-white hover:bg-gray-800 transition-colors shadow-sm text-center"
            >
              Next to Upload Scene &rarr;
            </Link>
          </div>
        </div>
        ...
        <div
          className={`text-center max-w-2xl mx-auto mb-8 p-4 rounded-xl border ${mode === "cubemap" ? "bg-gray-100 border-gray-200" : "bg-indigo-50 border-indigo-100"}`}
        >
          <p
            className={`text-sm font-medium ${mode === "cubemap" ? "text-gray-700" : "text-indigo-800"}`}
          >
            {mode === "cubemap"
        ...
        <form
          ...
          className="bg-white p-8 rounded-3xl shadow-sm border border-gray-200 relative"
        >
          {/* CUBEMAP UI */}
          {mode === "cubemap" && (
                ...
                <>
                        ...
                        if (!position)
                          return (
                            <div
                              ...
                              className="w-24 h-24 md:w-32 md:h-32 hidden sm:block opacity-0"
                            />
                          ); // Spacer
                        ...
                        return (
                          <div key={position} className="w-24 md:w-32">
                            <FileUploadBox
                              ...
                              onChange={(f) =>
                                handleGuidedFileChange(position, f)
                              }
                              small
                            />
                          </div>
                  ...
                  <div className="text-center mt-4 p-4 bg-blue-50 rounded-lg border border-blue-100">
                    <p className="text-sm text-blue-800 font-medium">
                      💡 Tip: Ensure ~30% overlap between adjacent photos.
                    </p>
                  </div>
                </>
              )}
                ...
                <div className="w-full">
                  <div className="border-2 border-dashed border-indigo-200 rounded-3xl p-12 flex flex-col items-center justify-center text-center bg-indigo-50/50 hover:bg-indigo-50 hover:border-indigo-400 transition-all cursor-pointer relative min-h-[320px] group">
                    ...
                    <h3 className="text-2xl font-bold text-gray-900 mb-3">
                      {autoFiles.length > 0
                        ? `${autoFiles.length} Photos Selected`
                        : "Drop Photos Here"}
                    </h3>
                    ...
                    {autoFiles.length > 0 && (
                      <div className="flex flex-wrap gap-3 justify-center max-w-3xl px-4 z-20 relative pointer-events-none">
                        {autoFiles.map((f, i) => (
                          <div
                            key={i}
                            className="px-3 py-1.5 bg-white rounded-lg text-xs font-medium border border-gray-200 shadow-sm text-gray-700 flex items-center gap-2 animate-fade-in pointer-events-auto hover:border-red-200 group/tag"
                          >
                            {f.name}
                            <button
                              type="button"
                    ...
                    {autoFiles.length === 0 && (
                      <button
                        type="button"
                        className="px-8 py-3 bg-indigo-600 text-white rounded-xl font-bold shadow-md shadow-indigo-200 pointer-events-none group-hover:bg-indigo-700 transition-colors"
                      >
                        Select Files
                      </button>
                    )}
          ...
          <div className="flex justify-between items-center pt-8 border-t border-gray-100 mt-auto">
            <div className="flex-1 pr-4">
              {status && (
                <span
                  className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium ${status.includes("Error") ? "bg-red-50 text-red-600" : "bg-green-50 text-green-700"}`}
                >
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                disabled={
                  mode === "cubemap"
                    ? !isCubeValid
                    : mode === "guided" && guidedMode === "manual"
                      ? !isGuidedValid
                      : mode === "guided" && guidedMode === "auto"
                        ? !isAutoValid
                        : true
                }
                className={`
        ...
        {file ? (
          <>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/80 to-transparent pointer-events-none">
              <p className="text-[10px] text-white/90 font-medium truncate px-1 text-center">
                {file.name}
              </p>
            </div>
          </>
